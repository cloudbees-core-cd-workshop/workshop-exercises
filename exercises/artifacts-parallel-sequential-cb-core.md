...
5. By default, the `preserveStashes` step will only preserver 1 run, but it provides the `buildCount` parameter to set a range from 1 up to 50 runs to preserve. We will preserve 2 runs to match the `buildDiscarder` policy configured for our Team Masters. 
